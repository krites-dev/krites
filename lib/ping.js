...
const ENDPOINT = "https://krites.dev/ping";
const VERSION = require("../.claude-plugin/plugin.json").version;
const CAP_MS = 2000;
const WEEK_MS = 7 * 24 * 60 * 60 * 1000;
const FILE = "telemetry.json";

const OFFLINE = "Telemetry is switched off: KRITES_PING_ENDPOINT is set to off, so nothing is asked and nothing is sent.";
const ON = "Telemetry is on: a ping goes out now and once a week. Run /krites:telemetry off to stop it.";
const OFF = "Telemetry is off. Nothing is sent. Run /krites:telemetry on to allow it.";
const QUESTION = [
  "Krites can send a ping to krites.dev now and once a week: a random install id, the plugin version, the operating system and a timestamp.",
...
// No endpoint means no question, no state file and nothing sent.
const endpointOf = () => {
  const endpoint = process.env.KRITES_PING_ENDPOINT || ENDPOINT;
  return endpoint.trim().toLowerCase() === "off" ? "" : endpoint;
};
