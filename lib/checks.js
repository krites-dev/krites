...
function childEnv(extra) {
  ...
  for (const [name, value] of Object.entries(extra)) {
    // Windows names ignore case: a PATH set beside an inherited Path would leave the winner to the order of the block.
    if (WINDOWS) for (const have of Object.keys(env)) if (have.toUpperCase() === name.toUpperCase()) delete env[have];
    env[name] = value;
  }
  return env;
}

function runCheck(argv, root, env, onChild) {
  return new Promise((resolve) => {
    const quote = (arg) => (/[\s"]/.test(arg) ? `"${arg}"` : arg);
    const options = { cwd: root, env, stdio: ["ignore", "pipe", "pipe"], windowsHide: true };
...
async function runChecks({ commands, env = {}, root, deadlineMs }) {
  ...
  const work = (async () => {
    for (const argv of commands) {
      ...
      running = argv;
      started = Date.now();
      const { code, out } = await runCheck(argv, root, merged, (spawned, read) => {
        child = spawned;
        partial = read;
      });
