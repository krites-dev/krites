...
const TABLES = { checks: ["commands", "timeout_seconds", "max_blocks"], protect: ["globs"], receipts: ["dir"] };
...
function parseToml(text) {
  ...
  for (;;) {
    ...
    if (src[i] === "[") {
      i += 1;
      const name = bareKey();
      if (name === null || src[i] !== "]") return fail("a table header is [checks], [protect] or [receipts]", at);
      i += 1;
      ...
      if (!(table === null ? TOP_KEYS : TABLES[table]).includes(key)) return fail(`unknown key ${full}`, at);
      if (seen.has(full)) return fail(`${full} appears twice`, at);
      seen.add(full);
      const parsed = readValue(0);
      if (!parsed.ok) return parsed;
      if (table === null) value[key] = parsed.value;
      else (value[table] = value[table] || {})[key] = parsed.value;
      lines[full] = at;
...
function validate(parsed) {
  ...
  const items = checks.commands === undefined ? [] : checks.commands;
  if (!Array.isArray(items)) return bad("checks.commands", "checks.commands is a list of commands");
  const itemLines = lines["checks.commands[]"] || [];
  const commands = [];
  for (const [index, item] of items.entries()) {
    const { argv, error } = toArgv(item);
    if (error) return badAt(itemLines[index] || at("checks.commands"), error);
    commands.push(argv);
  }

  return {
    ok: true,
    config: { version: 1, checks: { commands, timeout_seconds: timeout, max_blocks: maxBlocks }, protect: { globs }, receipts: { dir } },
  };
}
...
module.exports = { DEFAULT_MAX_BLOCKS, findRoots, loadConfig, parseToml, protectedGlobs };
