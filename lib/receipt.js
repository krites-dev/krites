...
const SCHEMA = "krites.receipt/0.2";
...
function agrees(run) {
  ...
  if (codes.length > run.commands.length) return false;
  ...
  if (run.verdict === "passed") return codes.length > 0 && codes.length === run.commands.length && codes.every((code) => code === 0);
  if (run.verdict === "no_checks") return run.commands.length === 0;
...
function gateWrote(run) {
  ...
  const ok = (check) => isObject(check) && keysAre(check, CHECK_KEYS) && isText(check.command) && isText(check.tail) && isCount(check.duration_ms);
  if (!isList(run.checks, (check) => ok(check) && (check.exit_code === null || Number.isInteger(check.exit_code)))) return false;
  return agrees(run);
}
...
function markdown(receipt) {
  ...
  const rows = (receipt.checks || []).map((check) => `| ${check.command} | ${check.exit_code} | ${check.duration_ms} |`);
  return `${[
    "# Krites receipt",
    "",
    `Verdict: ${receipt.verdict}`,
    `Reason: ${receipt.reason === null ? "none" : receipt.reason}`,
    "",
    ...
    `Config hash: ${receipt.config_hash}`,
    "",
...
module.exports = { build, existing, write };
